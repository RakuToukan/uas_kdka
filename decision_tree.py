...
def train_and_evaluate(avg_rgb, output_dir="output"):
    ...
    print("\n>>> Evaluasi Model")
    print("F-1 Score       : ", f1_score       (y_test, y_pred, average="weighted", zero_division=0))
...
def run_mosaic(target_path, category, output_path, grid_m=64, grid_n=64, output_dir="output"):
    ...
    if folder is None:
        raise ValueError(
            ...
            f"Pilihan: {list(FOLDER_CATEGORY.keys())}"
        )
    ...
    if os.path.exists(cache_path):
        ...
        print(f"[1/4] baca dataset dari {folder}")
